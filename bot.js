...
else if(view==="a_main"){t="🛠 Админ панель";kb={inline_keyboard:[[{text:"💸 Цены тарифов",callback_data:"a:t"}],[{text:"🎞 GIF",callback_data:"a:g"}],[{text:"📨 Рассылка",callback_data:"a:b"}],[{text:"💳 Способы оплаты",callback_data:"a:p"}],[{text:"🤝 Партнёрские настройки",callback_data:"a:r"}],[{text:"⬅️ Назад",callback_data:"v:home"}]]};}
...
async function handleAdminState(msg){
  ...
  const row=db.prepare("SELECT * FROM admin_states WHERE admin_tg_id=?").get(aid);if(!row)return false;
  const text=String(msg.text||"").trim(),chatId=Number(msg.chat?.id||0);
  if(text==="/cancel"){db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);await tg("sendMessage",{chat_id:chatId,text:"Отменено."});return true;}
  if(row.state==="tariff_price"){const n=Number(text);if(!Number.isFinite(n)||n<=0){await tg("sendMessage",{chat_id:chatId,text:"Введите корректную цену."});return true;}db.prepare("UPDATE tariffs SET price_rub=? WHERE code=?").run(Math.round(n),row.payload);db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);await tg("sendMessage",{chat_id:chatId,text:"Цена обновлена."});return true;}
  if(row.state==="gif"){let v=text;if(msg.animation?.file_id)v=msg.animation.file_id;if(!v){await tg("sendMessage",{chat_id:chatId,text:"Отправьте GIF или file_id."});return true;}setSetting(row.payload,v);db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);await tg("sendMessage",{chat_id:chatId,text:"Сохранено."});return true;}
  if(row.state==="pay_methods"){setSetting("payment_methods",text);db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);await tg("sendMessage",{chat_id:chatId,text:"Обновлено."});return true;}
  if(row.state==="broadcast"){db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);const ids=db.prepare("SELECT tg_id FROM users").all();let ok=0,fail=0;for(const u of ids){try{const g=setting("gif_broadcast","");if(g)await tg("sendAnimation",{chat_id:u.tg_id,animation:g,caption:text,parse_mode:"HTML"});else await tg("sendMessage",{chat_id:u.tg_id,text,parse_mode:"HTML"});ok++;}catch{fail++;}}await tg("sendMessage",{chat_id:chatId,text:`Рассылка завершена. Успешно: ${ok}, ошибок: ${fail}`});return true;}
  if(row.state==="ref_percent"){const n=Number(text);if(!Number.isFinite(n)||n<0||n>100){await tg("sendMessage",{chat_id:chatId,text:"Введите 0..100"});return true;}setSetting("ref_percent",Math.round(n));db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);await tg("sendMessage",{chat_id:chatId,text:"Ставка обновлена."});return true;}
  if(row.state==="ref_min"){const n=Number(text);if(!Number.isFinite(n)||n<0){await tg("sendMessage",{chat_id:chatId,text:"Введите сумму в рублях."});return true;}setSetting("ref_withdraw_min",Math.round(n));db.prepare("DELETE FROM admin_states WHERE admin_tg_id=?").run(aid);await tg("sendMessage",{chat_id:chatId,text:"Минимум обновлён."});return true;}
  return false;
}
...
async function handleCallback(q){
  ...
  if(data==="ref:p"){const u=user(uid);await tg("sendMessage",{chat_id:chatId,text:`Способ вывода: ${u.payout_method||"не задан"}\nРеквизиты: ${u.payout_details||"не указаны"}`});await tg("answerCallbackQuery",{callback_query_id:q.id,text:"Отправлено",show_alert:false});return;}
  if(data==="ref:w"){const min=Number(setting("ref_withdraw_min","1000"))||1000,u=user(uid);if(Number(u.balance_rub)<min){await tg("answerCallbackQuery",{callback_query_id:q.id,text:`Минимум: ${rub(min)}`,show_alert:true});return;}await tg("sendMessage",{chat_id:chatId,text:"Заявка на вывод отправлена администратору."});await tg("sendMessage",{chat_id:ADMIN_ID,text:`Запрос на вывод от ${uid}. Баланс: ${rub(u.balance_rub)}.`});await tg("answerCallbackQuery",{callback_query_id:q.id,text:"Заявка отправлена",show_alert:false});return;}

  if(data.startsWith("g:p:")){await render(uid,chatId,msgId,"gift_users",{code:data.split(":")[2],page:0});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  ...
  if(data.startsWith("a:te:")){db.prepare("INSERT INTO admin_states(admin_tg_id,state,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(admin_tg_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=excluded.updated_at").run(uid,"tariff_price",data.split(":")[2],now());await tg("sendMessage",{chat_id:chatId,text:"Введите новую цену тарифа.\n/cancel для отмены."});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:g"){await render(uid,chatId,msgId,"a_gif");await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data.startsWith("a:ge:")){db.prepare("INSERT INTO admin_states(admin_tg_id,state,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(admin_tg_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=excluded.updated_at").run(uid,"gif",data.split(":")[2],now());await tg("sendMessage",{chat_id:chatId,text:"Отправьте GIF или file_id.\n/cancel для отмены."});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:b"){await render(uid,chatId,msgId,"a_bcast");await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:bs"){db.prepare("INSERT INTO admin_states(admin_tg_id,state,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(admin_tg_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=excluded.updated_at").run(uid,"broadcast","",now());await tg("sendMessage",{chat_id:chatId,text:"Отправьте текст рассылки.\n/cancel для отмены."});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:p"){await render(uid,chatId,msgId,"a_pay");await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:pe"){db.prepare("INSERT INTO admin_states(admin_tg_id,state,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(admin_tg_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=excluded.updated_at").run(uid,"pay_methods","",now());await tg("sendMessage",{chat_id:chatId,text:"Отправьте новый текст способов оплаты.\n/cancel для отмены."});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:r"){await render(uid,chatId,msgId,"a_ref");await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:rp"){db.prepare("INSERT INTO admin_states(admin_tg_id,state,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(admin_tg_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=excluded.updated_at").run(uid,"ref_percent","",now());await tg("sendMessage",{chat_id:chatId,text:"Введите новую ставку в процентах (0..100)."});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
  if(data==="a:rm"){db.prepare("INSERT INTO admin_states(admin_tg_id,state,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(admin_tg_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=excluded.updated_at").run(uid,"ref_min","",now());await tg("sendMessage",{chat_id:chatId,text:"Введите минимальную сумму вывода."});await tg("answerCallbackQuery",{callback_query_id:q.id});return;}
